...
function ARView({ modelUrl, visible, onClose, modelId }) {
  ...
  // Update AR URL when model changes
  useEffect(() => {
    if (visible && modelUrl && modelId) {
      try {
        const directModelUrl = storage.getFileDownload('67ec002100025524dd4b', modelId);
        ...
        setError('Failed to generate AR view. Please try again.');
      }
    } else if (visible && (!modelUrl || !modelId)) {
      setError('No model available. Please upload a model in the main viewer.');
    }
  }, [visible, modelUrl, modelId]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75">
      <div className="bg-white rounded-lg p-4 w-full max-w-4xl h-5/6 flex flex-col">
        ...
        {/* Show model preview if model is available */}
        {modelUrl && modelId ? (
          <div className="flex-grow relative">
            <model-viewer
              ref={modelViewerRef}
          ...
          <div className="flex-grow flex items-center justify-center text-gray-700">
            <p>No model loaded. Please upload a model in the main viewer.</p>
          </div>
        )}
        ...
        {shareVisible && (
          <div className="absolute top-20 right-4 bg-white p-4 rounded-lg shadow-lg z-10 w-80">
            ...
            <div className="text-xs text-gray-500">
              Share this link with others to view this model in AR
            </div>
          </div>
        )}
        ...
        <div className="mt-4 text-gray-700 text-sm">
          <p>Instructions:</p>
          <ul className="list-disc pl-5">
            <li>Upload a model in the main viewer to enable AR mode</li>
            <li>Click "View in AR" to launch AR on supported devices</li>
            <li>Use pinch gestures to resize the model</li>
            <li>Use single finger to rotate the model</li>
            <li>Share the QR code or link to view on other devices</li>
          </ul>
...
export default function ModelViewer({ modelUrl: initialModelUrl, config }) {
  const [modelSize, setModelSize] = useState([1, 1, 1]);
  const [modelPosition, setModelPosition] = useState([0, 0, 0]);
  const [showDimensions, setShowDimensions] = useState(false);
  const [screenshotPreview, setScreenshotPreview] = useState(null);
  ...
  useEffect(() => {
    try {
      ...
      if (storedModelUrl && storedModelId) {
        setUploadedModelUrl(storedModelUrl);
        setModelId(storedModelId);
      } else if (initialModelUrl) {
        setUploadedModelUrl(initialModelUrl);
      }
    } catch (err) {
      console.error('Error loading from localStorage:', err);
      setError('Failed to load saved model. Please upload a new one.');
    }
  }, [initialModelUrl]);
  ...
  const handleModelUploaded = (url, id) => {
    try {
      ...
      setError(null); // Clear any previous errors
      console.log('Model uploaded and saved to localStorage:', { url, id });
    } catch (err) {
  ...
  const baseDimensions = [Math.max(modelSize[0] * 2, 4), Math.max(modelSize[2] * 2, 4)];
  const cameraDistance = Math.max(modelSize[1] * 3, 5);
  ...
  return (
    <div className="w-full h-full flex flex-col">
      <div className="relative flex-grow">
        {error && (
          <div className="absolute top-4 left-4 z-10 bg-red-500 text-white p-3 rounded shadow">
            <p>{error}</p>
          </div>
        )}
        ...
        <Canvas
          shadows
          camera={{ position: [0, modelSize[1] * 1.5, cameraDistance], fov: 70 }}
          ref={canvasRef}
        >
          <color attach="background" args={[config.backgroundColor || '#f0f0f0']} />
